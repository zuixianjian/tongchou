...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          width: 100,
          render: (h, params) => {
            if (params.row.orderType == "ORDER_TYPE_COMMON") {
              return h("div", "统筹单");
            } else if (params.row.orderType == "ORDER_TYPE_EDIT") {
              return h("div", "批单");
            } else if (params.row.orderType == "ORDER_TYPE_HELP") {
              return h("div", "互助单");
            }
            return h("div", "其他业务");
          },
  ...
  methods: {
    ...
    getDataList () {
      ...
      getStatisticOrders(this.searchForm).then((res) => {
        this.loading = false;
        if (res.success) {
          this.statsticDatas = res.result.statisticOrderItemResps;
          if (res.result.shouldRecieveSum != null) {
            this.statsticDatas.push({
              orderId: "合计",
              shouldRecieve: res.result.shouldRecieveSum,
              shouldCleanRecieve: res.result.shouldCleanRecieveSum,
              realRecieve: res.result.realRecieveSum
            })
          }
