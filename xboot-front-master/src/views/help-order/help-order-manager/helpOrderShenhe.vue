    ...
    <Card>
      ...
      <Row>
        <Col :span="24">
          ...
          <span v-show="drop">
            <base-info ref="baseinfo" op_type="check" :baseInfo="form" /> </span
        ></Col>
      </Row>
      <Row>
        <Col :span="24">
          ...
          <span v-show="dropDamage">
            <damage-list ref="damageList" op_type="check" :helpOrder="form" />
          </span>
        </Col>
      </Row>
      <Row>
      ...
      <div class="colContent">
        ...
        <Button
          type="primary"
          @click="checksuccess"
          v-has="'helpOrderCheckSuccess'"
          >审核通过</Button
        >
        <Button type="dashed" @click="checkFail" v-has="'helpOrderCheckFail'"
          >退回调查</Button
        >
      </div>
...
export default {
  ...
  methods: {
    ...
    checksuccess () {
      //审核通过
      helperOrderCheckSuccess({ helpOrderId: this.form.id }).then((resp) => {
        if (resp.success) {
    ...
    checkFail () {
      //退回调查
      helperOrderCheckFail({ helpOrderId: this.form.id }).then((resp) => {
        if (resp.success) {
