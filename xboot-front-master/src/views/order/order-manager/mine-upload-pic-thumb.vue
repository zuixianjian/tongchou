...
export default {
  ...
  props: {
    ...
    accept: {
      type: String,
      default: ".jpg, .jpeg, .png, .gif, .pdf, .docx, .doc",
    },
    uploadFileUrl: {
      type: String,
      default: "",
  ...
  methods: {
    ...
    downloadFile (data, u) {
      ...
      let prefix = u.slice(index)
      let url = window.URL.createObjectURL(new Blob([data]));
      let link = document.createElement("a");
      link.style.display = "none";
      link.href = url;
      link.setAttribute("download", u);
    ...
    handleSuccess (res, file) {
      if (res.success) {
        let realName = res.result;
        // 单张图片处理
        if (!this.multiple && this.uploadList.length > 0) {
          // 删除第一张
          this.uploadList.splice(0, 1);
          this.uploadNameList.splice(0, 1);
        }
        if (realName.indexOf(".jpg") != -1 || realName.indexOf(".jpeg") != -1 || realName.indexOf(".png") != -1 || realName.indexOf(".gif") != -1) {
          getFile(res.result).then((resp) => {
            ...
            reader.onload = (e) => {
              ...
              this.uploadNameList.push(realName);
              // 返回组件值
              this.returnValue();
            };
    ...
    returnValue () {
      ...
      if (!this.multiple) {
        // 单张
        let v = this.uploadNameList[0];
        this.$emit("input", v);
        this.$emit("on-change", v);
      } else {
        let v = [];
        this.uploadNameList.forEach((e) => {
          v.push(e);
        });
        this.$emit("input", v);
        this.$emit("on-change", v);
