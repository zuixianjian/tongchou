  ...
  watch: {
    ...
    $route (to, from) {
      if (to.name == "orderedit") {
        let of = this.$route.query.order;
        if (of.order.feeFloatRate != null) {
          of.order.feeFloatRate = parseFloat(of.order.feeFloatRate);
        }
        if (of.order.isCheckCar == null) {
          of.order.isCheckCar = false;
        }
        this.order = of;
        this.orderStatus = this.order.order.orderStatus
      }
