    ...
    >
        ...
        <Col span="3">
          <Form-item label="期限类型" :label-width="80" prop="timeLimitType">
            <Select
              v-bind:class="{
                editColor:
                  form.order.timeLimitType != oOrder.order.timeLimitType,
              }"
        ...
        <Col span="3">
          <Form-item label="自然月数" :label-width="80" prop="month">
            <Select
              ...
              disabled
              v-bind:class="{
                editColor: form.order.month != oOrder.order.month,
              }"
              readonly
            >
              <Option
        ...
        <Col span="3">
          <Form-item label="短期折扣" prop="shortDiscount">
            <InputNumber
              ...
              v-bind:class="{
                editColor:
                  form.order.shortDiscount != oOrder.order.shortDiscount,
              }"
            ></InputNumber>
          </Form-item>
        </Col>
        ...
        <Col span="4">
          <FormItem label="被统筹人" prop="clientName" ref="clientName">
            <div style="display: flex">
              <Input
                v-bind:class="{
                  editColor: form.order.clientName != oOrder.order.clientName,
                }"
        ...
        <Col span="6">
          <FormItem label="办理人" prop="handleName">
            <div style="display: flex">
              <Input
                v-bind:class="{
                  editColor: form.order.handleName != oOrder.order.handleName,
                }"
        ...
        <Col span="8">
          <FormItem label="驾驶证车主" prop="runCardName">
            <div style="display: flex">
              <Input
                v-bind:class="{
                  editColor: form.order.runCardName != oOrder.order.runCardName,
                }"
        ...
        <Col span="12">
          <FormItem label="车型" prop="carTypeName" ref="carTypeName">
            <div style="display: flex">
              <Input
                v-bind:class="{
                  editColor: form.order.carTypeName != oOrder.order.carTypeName,
                }"
      ...
      <Row type="flex" justify="center">
        <Col span="4">
          <FormItem label="车型种类" prop="carTypeA">
            <Select
              v-bind:class="{
                editColor: form.order.carTypeA != oOrder.order.carTypeA,
              }"
        ...
        <Col span="4">
          <FormItem label="细分类目" prop="carTypeB">
            <Select
              v-bind:class="{
                editColor: form.order.carTypeB != oOrder.order.carTypeB,
              }"
              v-model="form.order.carTypeB"
              placeholder="请选择"
              ref="carTypeB"
              key="carTypeB"
              v-if="carTypeBComponentKey"
              style="width: 120px"
            >
              <Option
        ...
        <Col span="4">
          <FormItem label="行驶证类型" prop="runCardCarType">
            <Select
              v-bind:class="{
                editColor:
                  form.order.runCardCarType != oOrder.order.runCardCarType,
              }"
      ...
      <Row type="flex" justify="center">
        <Col span="4">
          <FormItem label="车系分类" prop="carSieralType">
            <Select
              v-bind:class="{
                editColor:
                  form.order.carSieralType != oOrder.order.carSieralType,
              }"
        ...
        <Col span="4">
          <FormItem label="所属性质" prop="belongType">
            <Select
              v-bind:class="{
                editColor: form.order.belongType != oOrder.order.belongType,
              }"
        ...
        <Col span="4">
          <FormItem label="使用性质" prop="useType">
            <Select
              v-bind:class="{
                editColor: form.order.useType != oOrder.order.useType,
              }"
          ...
          <FormItem label="核定载重" prop="loadCapacity">
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor:
                    form.order.loadCapacity != oOrder.order.loadCapacity,
                }"
                :max="100000"
                :min="0"
        ...
        <Col span="4">
          <FormItem label="整备质量" prop="weight">
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor: form.order.weight != oOrder.order.weight,
                }"
                :max="100000"
                :min="0"
        ...
        <Col span="4">
          <FormItem label="功率" prop="kws">
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor: form.order.kws != oOrder.order.kws,
                }"
                :max="100000"
                :min="0"
          ...
          <FormItem label="排量(升)" prop="displacement">
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor:
                    form.order.displacement != oOrder.order.displacement,
                }"
                :max="100000"
                :min="0"
        ...
        <Col span="3">
          <FormItem label="座位数" prop="siteCount">
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor: form.order.siteCount != oOrder.order.siteCount,
                }"
                :max="100000"
                :min="0"
          ...
          <FormItem label="新车购置价" prop="buyPrice">
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor:
                    form.order.calculateMonthOldPrice !=
                    oOrder.order.calculateMonthOldPrice,
                }"
          ...
          <FormItem label="月折旧率" prop="monthOldRate">
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor:
                    form.order.monthOldRate != oOrder.order.monthOldRate,
                }"
                :max="100000"
                :min="0"
          ...
          <FormItem label="车牌号码" prop="carNo">
            <Row>
              ...
              <Col span="24">
                <Input
                  v-bind:class="{
                    editColor: form.order.carNo != oOrder.order.carNo,
                  }"
          ...
          <FormItem label="初登日期" prop="firstLogDate">
            ...
            >
              <Date-picker
                v-bind:class="{
                  editColor:
                    form.order.firstLogDate != oOrder.order.firstLogDate,
                }"
                type="datetime"
                placeholder="选择初登日期"
                style="width: 300px"
          ...
          <FormItem label="折旧后车价" prop="monthOldRatePrice">
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor:
                    form.order.monthOldRatePrice !=
                    oOrder.order.monthOldRatePrice,
                }"
                style="width: 300px"
                :max="100000"
                :min="0"
          ...
          <FormItem label="协商车价" prop="negotiatePrice">
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor:
                    form.order.negotiatePrice != oOrder.order.negotiatePrice,
                }"
                style="width: 300px"
                :max="1000000000000"
                :min="0"
      ...
      <Row type="flex" justify="center">
        <Col span="6">
          <FormItem label="发动机号" prop="motorNo">
            <div style="display: flex">
              <Input
                v-model="form.order.motorNo"
                style="margin-right: 10px"
                v-bind:class="{
                  editColor: form.order.motorNo != oOrder.order.motorNo,
                }"
              />
            </div>
          </FormItem>
        ...
        <Col span="6">
          <FormItem label="车架号" prop="carframeNo">
            <div style="display: flex">
              <Input
                v-bind:class="{
                  editColor: form.order.carframeNo != oOrder.order.carframeNo,
                }"
      ...
      <!-- <Row type="flex" justify="center">
        <div style="display: flex">
          <Checkbox
            v-bind:class="{
              editColor: form.order.isCheckCar != oOrder.order.isCheckCar,
            }"
            v-model="form.order.isCheckCar"
            >需要验车</Checkbox
          >
        </div>
      </Row> -->
    </Form>
    <Form v-if="form.order && oOrder.order">
      ...
      >
        <Col span="3">
          <Form-item
            :label-width="0"
            :prop="item.index + 'isChecked'"
            v-bind:class="{
              editColor: item.isChecked != oOrder.orderDetails[index].isChecked,
            }"
          >
        ...
        <Col span="2">
          ...
          >
            <Select
              v-bind:class="{
                editColor: item.everyFee != oOrder.orderDetails[index].everyFee,
              }"
        ...
        <Col span="1">
          ...
          >
            <Select
              v-bind:class="{
                editColor:
                  item.siteCount != oOrder.orderDetails[index].siteCount,
              }"
          ...
          >
            <Select
              v-bind:class="{
                editColor:
                  item.insuranceMoney !=
                  oOrder.orderDetails[index].insuranceMoney,
              }"
        ...
        <Col span="2">
          ...
          >
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor: item.baseFee != oOrder.orderDetails[index].baseFee,
                }"
        ...
        <Col span="2">
          ...
          >
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor: item.feeRate != oOrder.orderDetails[index].feeRate,
                }"
          ...
          >
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor:
                    item.actualFee != oOrder.orderDetails[index].actualFee,
                }"
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor:
                    item.deductibleFee !=
                    oOrder.orderDetails[index].deductibleFee,
                }"
                @on-change="calculateFees1"
                :max="100000"
                :min="0"
      ...
      <Row type="flex" justify="center" :gutter="8">
        <Col span="3">
          <FormItem label="统筹总金额(元)" prop="totalFee">
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor: form.order.totalFee != oOrder.order.totalFee,
                }"
                :max="100000"
                :min="0"
          ...
          <FormItem label="价格浮动因素(优惠)" prop="feeFloatReason">
            <div style="display: flex">
              <Input
                v-bind:class="{
                  editColor:
                    form.order.feeFloatReason != oOrder.order.feeFloatReason,
                }"
          ...
          <FormItem label="价格浮动" prop="feeFloatRate">
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor:
                    form.order.feeFloatRate != oOrder.order.feeFloatRate,
                }"
                :max="100000"
                :min="0"
        ...
        <Col span="4">
          <FormItem label="浮动后统筹总金额" prop="feeFloat">
            ...
            >
              <InputNumber
                v-bind:class="{
                  editColor: form.order.feeFloat != oOrder.order.feeFloat,
                }"
                @on-change="calculateFees1"
                :max="100000"
                :min="0"
        ...
        <Col span="4">
          <FormItem label="补(退) 统筹费(元)" prop="feeDiffer">
            {{ feeDiffer }}
          </FormItem>
        </Col>
      </Row>
          ...
          <FormItem label="备注信息" prop="remark">
            ...
            >
              <Input
                ...
                style="width: 700px"
                v-bind:class="{
                  editColor: form.order.remark != oOrder.order.remark,
                }"
              />
            </Tooltip>
          </FormItem>
      ...
      <Row type="flex" justify="center" :gutter="8">
        <Col span="12">
          <FormItem label="修改批文" prop="orderAgree">
            <div style="display: flex">
              <Input
                v-bind:class="{
                  editColor: form.order.orderAgree != oOrder.order.orderAgree,
                }"
                ...
                :rows="6"
              />
              <Button icon="md-list" @click="generatePiwen">生成</Button>
            </div>
          </FormItem>
        </Col>
      </Row>
...
<script>
import mineUploadPicThumb from "../order-manager/mine-upload-pic-thumb.vue";
...
export default {
  ...
  computed: {
    ...mapState(["dict"]),
    feeDiffer: function () {
      return (this.form.order.feeFloat - this.originOrder.order.feeFloat).toFixed(2)
    }
  },
  watch: {
    ...
    originOrder: function () {


      if (this.originOrder != null) {
        this.oOrder = this.originOrder;
      ...
      if (this.oOrder.orderDetails == null) {
        this.oOrder.orderDetails = [];
      }
      //你需要执行的代码
      //处理一下数据,将未选择的统筹添加上
      if (this.$store.state.dict.feeTypes != null) {
        let str = JSON.stringify(this.$store.state.dict.feeTypes);
        let data = JSON.parse(str);
        this.oOrder.orderDetails.forEach((item) => {
          //遍历查询是否已勾选所有统筹类型
          let ofIndex = -1;
          data.forEach((i, index) => {
            if (i.value == item.feeType) {
        ...
        data.forEach((item) => {
          this.originOrder.orderDetails.push({
            feeType: item.value,
            isChecked: false,
        ...
        this.originOrder.orderDetails = this.sortByKey(this.originOrder.orderDetails, 'sortOrder')
      }
    },
  },
  methods: {
    ...
    generatePiwen () {
      ...
      var piwen = "经被统筹人申请，统筹人同意自" + s1 + " 00:00:00" + "起，对统筹单号为" + this.form.order.id + "的统筹单作如下批改：" + " \n ";
      var piwen1 = "（1）变更客户信息" + " \n ";
      //变更客户信息
      if (this.form.order.clientName != this.oOrder.order.clientName) {
        //被统筹人变更
        piwen1 += "被统筹人姓名由:" + this.oOrder.order.clientName + "变更为:" + this.form.order.clientName +
          "证件号码由:" + this.oOrder.order.clientCardNo + "变更为:" + this.form.order.clientCardNo + ";"
      }
      if (this.form.order.handleName != this.oOrder.order.handleName) {
        //办理人变更
        piwen1 += "办理人姓名由:" + this.oOrder.order.handleName + "变更为:" + this.form.order.handleName +
          "证件号码由:" + this.oOrder.order.handleCardNo + "变更为:" + this.form.order.handleCardNo + ";"
      }
      if (this.form.order.runCardName != this.oOrder.order.runCardName) {
        //行驶证车主变更
        piwen1 += "行驶证车主由:" + this.oOrder.order.runCardName + "变更为:" + this.form.order.runCardName +
          "证件号码由:" + this.oOrder.order.runCardCardNo + "变更为:" + this.form.order.runCardCardNo + ";"
      }
      var piwen2 = " \n（2）车辆信息变更" + " \n ";

      //车辆信息变更
      if ((this.form.order.carTypeA != this.oOrder.order.carTypeA) || (this.form.order.carTypeB != this.oOrder.order.carTypeB)) {
        //车辆种类变更
        piwen2 += "车辆种类由:" + this.getCommonDictData(this.oOrder.order.carTypeA, this.$store.state.dict.carTypeAs) + this.getCommonDictData(this.oOrder.order.carTypeB, this.$store.state.dict.carTypeBs) +
          "变更为:" + this.getCommonDictData(this.form.order.carTypeA, this.$store.state.dict.carTypeAs) + this.getCommonDictData(this.form.order.carTypeB, this.$store.state.dict.carTypeBs) + ";"
      }

      if (this.form.order.carTypeName != this.oOrder.order.carTypeName) {
        //车型名称
        piwen2 += "车型名称由:" + this.oOrder.order.carTypeName + "变更为:" + this.form.order.carTypeName + ";"
      }
      if (this.form.order.carCode != this.oOrder.order.carCode) {
        //车型代码
        piwen2 += "车型代码由:" + this.oOrder.order.carCode + "变更为:" + this.form.order.carCode + ";"
      }
      if (this.form.order.displacement != this.oOrder.order.displacement) {
        //排量
        piwen2 += "排量由:" + this.oOrder.order.displacement + "变更为:" + this.form.order.displacement + ";"
      }
      if (this.form.order.loadCapacity != this.oOrder.order.loadCapacity) {
        //核定载重
        piwen2 += "核定载重由:" + this.oOrder.order.loadCapacity + "变更为:" + this.form.order.loadCapacity + ";"
      }
      if (this.form.order.weight != this.oOrder.order.weight) {
        //整备质量
        piwen2 += "整备质量由:" + this.oOrder.order.weight + "变更为:" + this.form.order.weight + ";"
      }
      if (this.form.order.kws != this.oOrder.order.kws) {
        //功率
        piwen2 += "功率由:" + this.oOrder.order.kws + "变更为:" + this.form.order.kws + ";"
      }
      if (this.form.order.buyPrice != this.oOrder.order.buyPrice) {
        //购置价
        piwen2 += "购置价由:" + this.oOrder.order.buyPrice + "变更为:" + this.form.order.buyPrice + ";"
      }
      if (this.form.order.carNo != this.oOrder.order.carNo) {
        //具体车牌号码
        piwen2 += "车牌号码由:" + this.oOrder.order.carNo + "变更为:" + this.form.order.carNo + ";"
      }
      if (this.form.order.useType != this.oOrder.order.useType) {
        //使用性质
        piwen2 += "使用性质由:" + this.getCommonDictData(this.oOrder.order.useType, this.$store.state.dict.useTypess) + "变更为:" + this.getCommonDictData(this.form.order.useType, this.$store.state.dict.useTypess) + ";"
      }
      if (this.form.order.belongType != this.oOrder.order.belongType) {
        //所属性质
        piwen2 += "所属性质由:" + this.getCommonDictData(this.oOrder.order.belongType, this.$store.state.dict.belongTypess) + "变更为:" + this.getCommonDictData(this.form.order.belongType, this.$store.state.dict.belongTypess) + ";"
      }
      if (this.form.order.runCardCarType != this.oOrder.order.runCardCarType) {
        //行驶证车辆类型
        piwen2 += "行驶证车辆类型由:" + this.getCommonDictData(this.oOrder.order.runCardCarType, this.$store.state.dict.runCardCarTypes) + "变更为:" + this.getCommonDictData(this.form.order.runCardCarType, this.$store.state.dict.runCardCarTypes) + ";"
      }
      if (this.form.order.carSieralType != this.oOrder.order.carSieralType) {
        //车系分类
        piwen2 += "车系分类由:" + this.getCommonDictData(this.oOrder.order.carSieralType, this.$store.state.dict.carSieralTypess) + "变更为:" + this.getCommonDictData(this.form.order.carSieralType, this.$store.state.dict.carSieralTypess) + ";"
      }
      if (this.form.order.siteCount != this.oOrder.order.siteCount) {
        //座位数
        piwen2 += "座位数由:" + this.getCommonDictData(this.oOrder.order.siteCount, this.$store.state.dict.siteCounts) + "变更为:" + this.getCommonDictData(this.form.order.siteCount, this.$store.state.dict.siteCounts) + ";"
      }
      if (this.form.order.firstLogDate != this.oOrder.order.firstLogDate) {
        //初登日期
        piwen2 += "初登日期由:" + this.oOrder.order.firstLogDate + "变更为:" + this.form.order.firstLogDate + ";"
      }
      if (this.form.order.beforeMonthOldRatePrice != this.oOrder.order.beforeMonthOldRatePrice) {
        //折旧前车价
        piwen2 += "折旧前车价由:" + this.oOrder.order.beforeMonthOldRatePrice + "变更为:" + this.form.order.beforeMonthOldRatePrice + ";"
      }
      if (this.form.order.monthOldRate != this.oOrder.order.monthOldRate) {
        //月折旧率
        piwen2 += "月折旧率由:" + this.oOrder.order.monthOldRate + "变更为:" + this.form.order.monthOldRate + ";"
      }
      if (this.form.order.monthOldRatePrice != this.oOrder.order.monthOldRatePrice) {
        //折旧后车价
        piwen2 += "折旧后车价由:" + this.oOrder.order.monthOldRatePrice + "变更为:" + this.form.order.monthOldRatePrice + ";"
      }
      if (this.form.order.negotiatePrice != this.oOrder.order.negotiatePrice) {
        //协商车价
        piwen2 += "协商车价由:" + this.oOrder.order.negotiatePrice + "变更为:" + this.form.order.negotiatePrice + ";"
      }
      if (this.form.order.motorNo != this.oOrder.order.motorNo) {
        //发动机号
        piwen2 += "发动机号由:" + this.oOrder.order.motorNo + "变更为:" + this.form.order.motorNo + ";"
      }
      if (this.form.order.carframeNo != this.oOrder.order.carframeNo) {
        //车架号
        piwen2 += "车架号由:" + this.oOrder.order.carframeNo + "变更为:" + this.form.order.carframeNo + ";"
      }
      var piwen3 = " \n（3）统筹项目与统筹费变更" + " \n ";
      //统筹项目修改
      var detailCount = this.form.orderDetails.length
      for (var i = 0; i < detailCount; i++) {
        if (this.form.orderDetails[i].isChecked != this.oOrder.orderDetails[i].isChecked) {
          //统筹项目修改  新增或减少
          if (this.form.orderDetails[i].isChecked) {
            //新增
            piwen3 += "增加" + this.getDictData(this.form.orderDetails[i].feeType) + "," + "统筹限额为" + this.form.orderDetails[i].insuranceMoney + "元," +
              "基础保费为" + this.form.orderDetails[i].baseFee + "元,费率为" + this.form.orderDetails[i].feeRate + ",不计免赔为" + this.form.orderDetails[i].deductibleFee +
              "元,应收保费" + this.form.orderDetails[i].actualFee.toFixed(2) + "元,统筹费变动" + this.form.orderDetails[i].actualFee.toFixed(2) + "元;"
          } else {
            //减少
            piwen3 += "减少" + this.getDictData(this.form.orderDetails[i].feeType) + "原统筹费" + this.oOrder.orderDetails[i].actualFee + "元,原不计免赔统筹费" + this.oOrder.orderDetails[i].deductibleFee +
              "元，统筹费变动-" + this.oOrder.orderDetails[i].actualFee + "元;"
          }
        } else {
          if (this.form.orderDetails[i].isChecked) {
            var edit = ""
            //判断数据有没有变更
            if (this.form.orderDetails[i].insuranceMoney != this.oOrder.orderDetails[i].insuranceMoney) {
              //保额变化
              edit += "原统筹限额为:" + this.oOrder.orderDetails[i].insuranceMoney + ";"
            }
            if (this.form.orderDetails[i].baseFee != this.oOrder.orderDetails[i].baseFee) {
              //基础保费
              edit += "原基础保费为:" + this.oOrder.orderDetails[i].baseFee + ";"
            }
            if (this.form.orderDetails[i].feeRate != this.oOrder.orderDetails[i].feeRate) {
              //费率
              edit += "原费率为:" + this.oOrder.orderDetails[i].feeRate + ";"
            }

            if (this.form.orderDetails[i].deductibleFee != this.oOrder.orderDetails[i].deductibleFee) {
              //不计免赔
              edit += "原不计免赔为:" + this.oOrder.orderDetails[i].deductibleFee + ";"
            }
            if (this.form.orderDetails[i].actualFee != this.oOrder.orderDetails[i].actualFee) {
              //应收保费
              piwen3 += "修改" + this.getDictData(this.form.orderDetails[i].feeType) + "," + edit + ",统筹费变动" + (this.form.orderDetails[i].actualFee - this.oOrder.orderDetails[i].actualFee).toFixed(2) + "元;"
            }
          }
        }
      }
      var piwen4 = "\n";
      //统筹总额变化
      if (this.form.order.totalFee != this.oOrder.order.totalFee) {
        //统筹总金额
        piwen4 += "原统筹总金额为" + this.oOrder.order.totalFee + "元;"
      }
      if (this.form.order.feeFloatRate != this.oOrder.order.feeFloatRate) {
        //费率浮动比例(优惠)
        piwen4 += "原费率浮动比例为" + this.oOrder.order.feeFloatRate + ";"
      }
      if (this.form.order.feeFloat != this.oOrder.order.feeFloat) {
        //浮动后统筹金额
        piwen4 += "原浮动后统筹金额为" + this.oOrder.order.feeFloat + ";"
      }
      piwen4 += "本单统筹费变化:" + (this.form.order.totalFee - this.oOrder.order.totalFee).toFixed(2) + "元;" + "费率浮动后应收统筹费变化" + (this.form.order.feeFloat - this.oOrder.order.feeFloat).toFixed(2) + "元;"
      var piwen5 = "\n 原统筹单所载其他条件不变，特此批注。";
      this.form.order.orderAgree = piwen + piwen1 + piwen2 + piwen3 + piwen4 + piwen5;
    },

    getCommonDictData (value, array) {
    ...
    refreshCarTypeB (res) {
      ...
      this.carTypeBComponentKey = false
      this.carTypeBComponentKey = true

    },
    //供父组件调用获取order数据
    getOrderDatas () {
      if (this.form.order.orderAgree == null || this.form.order.orderAgree.length == 0) {
        this.generatePiwen();
      }
      var that = this;
    ...
    onEveryFeeSelctChange (e, item) {
      ...
      if (item.feeType == "passenger") {
        if (item.everyFee != null && item.siteCount != null) {
          ...
          item.actualFee = (item.baseFee * parseFloat(item.feeRate)) / 100;
        }
      }
      this.calculateFees1();
    },
