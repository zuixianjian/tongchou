...
<template>
  <div class="search">
    <Card>
      <Row @keydown.enter.native="handleSearch">
        <Form ref="searchForm" :model="searchForm" inline :label-width="140">
          ...
          <Form-item label="统筹单id" prop="parentId">
            <Input
              type="text"
          ...
          <Form-item label="批单id" prop="queryId">
            <Input
              type="text"
          ...
          <Form-item style="margin-left: -35px" class="br">
            <Button
              ...
              type="primary"
              icon="ios-search"
              v-has="'order-edit-manager-search'"
              >搜索</Button
            >
      ...
      <Row class="operation">
        ...
        <Button
          ...
          type="success"
          icon="md-checkmark"
          v-has="'order-edit-manager-auditresult'"
          >审核通过</Button
        >
        <Button
          @click="auditFailure"
        ...
        <Button
          @click="delAll"
          icon="md-trash"
          v-has="'order-edit-manager-delete'"
          >批量删除</Button
        >
        <Button
          @click="getDataList"
    ...
    >
      ...
      >
        <Row type="flex" justify="center">
          <Form-item label="车牌号码" prop="carNo">
            <Input
              type="text"
              v-model="modalSearchForm.carNo"
              clearable
              placeholder="请输入车牌号码"
              style="width: 200px"
            />
          </Form-item>
          <Form-item label="车架号" prop="carframeNo">
            <Input
              type="text"
              v-model="modalSearchForm.carframeNo"
              clearable
              placeholder="请输入车架号"
              style="width: 200px"
            />
          </Form-item>
          <Form-item label="统筹单号" prop="queryId">
            <Input
              type="text"
              v-model="modalSearchForm.queryId"
              clearable
              placeholder="请输入统筹单号"
              style="width: 200px"
            />
      ...
      <a-divider :dashed="true" style="color: #0f0">查询结果</a-divider>
      <Form
        ref="selectOrderModalResult"
        :model="selectOrderModalResultOrder"
        inline
        :label-width="70"
      >
        <Row type="flex" justify="center">
          <Form-item label="统筹单号" prop="id">
            <Input
              type="text"
              v-model="selectOrderModalResultOrder.id"
              clearable
              placeholder=""
              style="width: 200px"
            />
          ...
          <Form-item label="车架号" prop="carframeNo">
            <Input
              type="text"
              v-model="selectOrderModalResultOrder.carframeNo"
              clearable
              placeholder=""
              style="width: 200px"
            />
          </Form-item>
          <Form-item label="到期日" prop="endDate">
            <Date-picker
              type="datetime"
              placeholder=""
              ref="endDate"
              :value="selectOrderModalResultOrder.endDate"
              :editable="false"
              clearable
              disabled
            ></Date-picker>
    ...
    <Modal v-model="pidanView" title="批单预览" @on-ok="pidanDownload"  width="80%" ok-text="">
      <iframe  id='previewPdf' :src="'/pdf/web/viewer.html?file=' + pdfSrc" height="650"
                    width="100%">
            </iframe>
      <!-- <iframe
        :src="pdfSrc"
...
import orderDetail from "../order-manager/order-detail.vue";
import docView from "../order-manager/doc-view.vue";
import dictUtil from "@/libs/dictUtil";
export default {
  components: { orderDetail, docView },
  name: "order-edit-manager",
  data () {
    return {
      pidanView: false,
      pdfSrc: "",
      selectOrderModal: false,
      selectOrderModalResult: {},
      selectOrderModalResultOrder: {},
      modalSearchForm: {
      ...
      columns: [
        ...
        },
        {
          ...
          width: 300,
          render: (h, params) => {
            return h("div", params.row.order.parentId);
          },
        },
        {
          title: "统筹单状态",
            ...
            return h("div", [
              ...
              h(
                "Button",
                {
                  ...
                  directives: [
                    {
                      name: 'has',
                      value: 'order-edit-manager-delete'
                    }
                  ],
                  on: {
                    click: () => {
              ...
              h(
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      this.orderEditPrint(params.row);
                    },
                  },
                },
                "下载打印"
              ...
              h(
                "Button",
                {
                  ...
                  directives: [
                    {
                      name: 'has',
                      value: 'order-edit-manager-actionlog'
                    }
                  ],
                  on: {
                    click: () => {
  ...
  methods: {
    ...
    selectOrderOk () {
      if (this.selectOrderModalResultOrder.id == null) {
        this.$Message.error("未检索到相关统筹单")
        return
      }
      // 记录返回路由
      let query = { order: this.selectOrderModalResult, backRoute: this.$route.name };
      this.$router.push({
    ...
    handleSearchOrderModal () {
      //查询统筹单
      if ((this.modalSearchForm.carNo == null || this.modalSearchForm.carNo == '') &&
        (this.modalSearchForm.carframeNo == null || this.modalSearchForm.carframeNo == '') &&
        (this.modalSearchForm.queryId == null || this.modalSearchForm.queryId == '')) {
        this.$Message.error("请填入检索条件")
        return
      }
      queryOrder(this.modalSearchForm).then(
        (resp) => {
          if (resp.success) {
            if (resp.result != null)
              this.selectOrderModalResult = resp.result[0];
            this.selectOrderModalResultOrder = resp.result[0].order
          }
        }
      )
    },
    ...
    checkFailRemarkOk () {
      ...
      orderEditAudit({ ids: ids, auditResult: false, result: "不通过", remark: this.checkFailRemark }).then((res) => {
        this.$Modal.remove();
        if (res.success) {
    ...
    getDataList () {
      this.loading = true;
      // 请求后端获取表单数据 请自行修改接口
      getOrderEditListData(this.searchForm).then((res) => {
        this.loading = false;
        if (res.success) {
    ...
    orderEditPrint (v) {
      orderEditPrint({ orderId: v.order.id }).then((res) => {
        if (res.success) {
          this.getPdfBlob(res.result)
        }
    ...
    auditSuccess () {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          ids = ids.substring(0, ids.length - 1);
          orderEditAudit({ ids: ids, auditResult: true, result: "通过" }).then((res) => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.clearSelectAll();
