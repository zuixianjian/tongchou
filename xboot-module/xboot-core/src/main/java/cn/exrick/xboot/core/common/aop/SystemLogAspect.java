...
import cn.exrick.xboot.core.common.utils.ThreadPoolUtil;
...
public class SystemLogAspect {
    ...
    public void after(JoinPoint joinPoint){
        try {
            ...
            String username = "";
            String description = controllerMethodInfo.get("description").toString();
            String dataFlag = controllerMethodInfo.get("dataFlag").toString();
            String desc = controllerMethodInfo.get("desc").toString();
            int type = (int)controllerMethodInfo.get("type");
            String specialDesc = controllerMethodInfo.get("specialDesc").toString();
            ...
            if(esRecord){
                ...
                esLog.setDataFlag(dataFlag);
                esLog.setLogDesc(desc);
                esLog.setSpecialDesc(specialDesc);
                ...
                log.setDataFlag(dataFlag);
                log.setLogDesc(desc);
                log.setSpecialDesc(specialDesc);
                ipInfoUtil.getInfo(request, ObjectUtil.mapToStringAll(request.getParameterMap()));
    ...
    public static Map<String, Object> getControllerMethodInfo(JoinPoint joinPoint) throws Exception{
        ...
        for(Method method : methods) {
            ...
            description = AnnotationResolver.newInstance().resolver(joinPoint, method.getAnnotation(SystemLog.class).description()).toString();
            type = method.getAnnotation(SystemLog.class).type().ordinal();
            dataFlag = AnnotationResolver.newInstance().resolver(joinPoint, method.getAnnotation(SystemLog.class).dataFlag()).toString(); //利用AnnotationResolver进行解析
            desc = AnnotationResolver.newInstance().resolver(joinPoint, method.getAnnotation(SystemLog.class).desc()).toString();
            specialDesc = AnnotationResolver.newInstance().resolver(joinPoint, method.getAnnotation(SystemLog.class).specialDesc()).toString();
            map.put("description", description);
            map.put("type", type);
            map.put("dataFlag", dataFlag);
            map.put("desc",desc);
            map.put("specialDesc",specialDesc);
        }
        return map;
    }
